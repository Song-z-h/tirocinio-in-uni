...
def train(env_name="CartPole-v0", hidden_sizes=[32], lr=1e-2, 
          epoch=50, batch_size=5000, render=False, k_step_value_steps=30,
            lamda=0.97, gamma=0.99, clip_ratio=0.2):
    #define env
    ...
    def train_one_epoch(old_logp):
        ...
        while True:
            ...
            if done:
                ...
                if len(batch_obs) > batch_size:
                    ...
                    if old_logp is None:
                        old_logp = np.zeros(batch_size)
                        old_logp = torch.as_tensor(old_logp.copy(), dtype=torch.float32)
                    break

        batch_obs = batch_obs[:batch_size]
        batch_acts = batch_acts[:batch_size]
        batch_advs = batch_advs[:batch_size]
        batch_weights = batch_weights[:batch_size]
        ...
        optimizer.zero_grad()
        loss, old_logp = compute_loss(torch.as_tensor(batch_obs, dtype=torch.float32),
        ...
        return loss, batch_rews, batch_lens, old_logp.clone()
            
    for i in range(epoch):
        batch_loss, batch_rews, batch_lens, old_logp = train_one_epoch(old_logp)
