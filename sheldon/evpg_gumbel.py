...
class EVPG(torch.nn.Module):
    def __init__(self, obs_dim, act_dim, hidden_sizes, temperature):
        ...
        self.value_net = value_mlp(input_size=act_dim+obs_dim, hidden_size=hidden_sizes[0])
    ...
    def get_action(self, soft_logits):
        return torch.argmax(soft_logits).item()
    ...
    def forward(self, obs):
        ...
        #act = self.get_policy(obs).sample().item()
        obs_act = torch.cat((obs, act_prob), -1)
...
def train(env_name="CartPole-v0", hidden_sizes=[32], lr=1e-2, 
    ...
    def train_one_epoch():
        ...
        while True:
            ...
            act, val = evpg_net(to_tensor(obs))
...
if __name__ == "__main__":
    ...
    file_name = f'temp_evpg_gumbel_temp_decay_sim_{num_simulations}_{env_name}.txt'
    ...
    for i in range(num_simulations):
        all_rewards[i, :] = train(env_name=env_name, render=False, 
                                  ...
                                  hidden_sizes=[32], temperature=0.5, temp_decay=True)
